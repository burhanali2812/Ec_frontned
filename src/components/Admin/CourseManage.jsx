...
function CourseManage({ adminLoginType = "academy" }) {
  ...
  const { classOptions , courses, setCourses, teachers, fetchCourses} = useAppContext();
  ...
  console.log("formData.teacherAssignments", formData.teacherAssignments);
  ...
  return (
    <Sidebar>
      ...
      <section className="cm-toolbar mb-3">
        <button
          type="button"
          className="btn btn-dark cm-add-btn"
          onClick={() => {
            resetForm();
            setShowModal(true);
          }}
                            ...
                            {isSelected && (
                              <div className="cm-class-button-wrap">
                                {CLASS_OPTIONS.map((classData) => {
                                  const active = selectedClasses.includes(
                                    classData._id || classData,
                                  );
                                  return (
                                    <button
                                      key={classData._id || classData}
                                      type="button"
