...
function StudentManage({ adminLoginType = "academy" }) {
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (selectedClass) {
      filtered = filtered.filter(
        (student) => student.classInfo === selectedClass,
      );
    }
  ...
  const getStudentCountByClass = (classInfo) => {
    return students.filter((student) => student.classInfo === classInfo).length;
  };
        ...
        <section className="sm-class-filter-section mt-4 mb-4">
          <div className="sm-class-filter-grid">
            {CLASS_OPTIONS.map((classOption) => (
              ...
              >
                <div className="sm-class-title">{classOption}</div>
          ...
          <>
            <section className="sm-toolbar mb-3 mt-3">
              <button
                ...
                onClick={() => {
                  resetForm();
                  // Pre-select the class if one is already selected
                  if (selectedClass) {
                    setFormData((prev) => ({
                      ...prev,
