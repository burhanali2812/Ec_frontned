...
const CLASS_OPTIONS = classOptions || [];
  ...
  const availableTeachers = useMemo(() => {
    ...
    const teachers = assignments
      .filter((item) => {
        if (!String(form.classInfo).trim()) return true;
     return Array.isArray(item?.targetClasses)
  ? item.targetClasses.some(
      (cls) => String(cls?._id || cls) === String(form.classInfo)
    )
  : false;
      })
 ...
 ${item?.dayOfWeek || ""}`.toLowerCase();
  ...
  const handleEdit = (entry) => {
    ...
    setForm({
      ...
      classInfo: String(entry?.classInfo?._id || entry?.classInfo || ""),
