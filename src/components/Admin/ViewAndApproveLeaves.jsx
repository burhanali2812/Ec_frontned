...
function ViewAndApproveLeaves() {
  ...
  const handleStatusUpdate = async (leaveId, newStatus, studentId, applicant, fromDate, toDate) => {
    if (newStatus === "Rejected" && !rejectionReason.trim()) {
      alert("Please provide a rejection reason");
      return;
    }
    if(leaveId && newStatus === "Approved" && applicant === "Student" && studentId && fromDate && toDate){
      // Additional confirmation for approving student leave
      const confirmMsg = `Are you sure you want to approve this leave? This will mark the student as "On Leave" in attendance records from ${new Date(fromDate).toLocaleDateString()} to ${new Date(toDate).toLocaleDateString()}. with applicant ${applicant} and studentid ${studentId}`;
    ...
    try {
      ...
      const res = await axios.put(
        ...
        { headers: getAuthHeaders() },
         ...
         if (res?.data?.success) {
        if (applicant === "Student" && studentId && newStatus === "Approved") {
          const attendanceRes = await axios.put(
            `${API_BASE}/attendance/updateLeaveAttendance/${studentId}`,
            {
              startDate: fromDate,
              endDate: toDate,
            },
      ...
      if (res?.data?.success) {
        ...
        alert(res?.data?.message || "Failed to update leave application");
      }
    }
                  ...
                  <div className="detail-item">
                    ...
                    <div className="action-buttons">
                      <button
                        ...
                        onClick={() =>
                          handleStatusUpdate(selectedLeave._id, "Approved", selectedLeave.studentId, selectedLeave.applicant, selectedLeave.fromDate, selectedLeave.toDate)
                        }
                        disabled={updating}
                      >
                ...
                <div className="detail-item mt-3 p-3 border rounded bg-light">
                  ...
                  <div className="d-flex gap-2">
                    <button
                      ...
                      onClick={() =>
                        handleStatusUpdate(selectedLeave._id, "Rejected", selectedLeave.studentId, selectedLeave.applicant, selectedLeave.fromDate, selectedLeave.toDate)
                      }
